...
// Standalone bench-test mode: never returns. Reads single-char commands from
// UART (0/1/2=set BLUE/RED/GREEN solo, a=all on, x=all off) and reports the
// GPIO pin number driven by each command over serial, so you can correlate
// commands to whichever physical LED actually lights up on real hardware.
// Gated behind LED_BENCH_TEST in main.c -- see FLASH_LED_TEST.command.
