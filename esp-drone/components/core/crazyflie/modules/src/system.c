 ...
 * The blimp power-cycles mid-flight and there's only ONE USB cable, so we can't
 * be tethered when it dies. esp_reset_reason() persists the LAST reset cause
 * across the reboot, and plugging USB-CDC into a running S3 does NOT reset it —
 * so: fly untethered, let it die+reboot, then plug in and this 5 s heartbeat
 * tells you why it reset (BROWNOUT = battery/wiring sag; PANIC/WDT = firmware).
 */
...
static void resetHeartbeatTask(void *arg)
{
  const esp_reset_reason_t r = esp_reset_reason();   /* cause of THIS boot */

  /* If the last reset was a PANIC, the core dump (saved to the flash coredump
   * partition at crash time) tells us WHERE: crashing task + program counter.
   * Print it every beat so it's visible whenever USB gets plugged in. */
  /* static: the summary struct is far too big for a small task stack (putting
   * it on the stack overflowed -> panic -> boot loop). */
  static esp_core_dump_summary_t dump;
  const bool haveDump = (esp_core_dump_get_summary(&dump) == ESP_OK);

  for (;;) {
    DEBUG_PRINTI("HB up=%us last_reset=%s",
                 (unsigned)(xTaskGetTickCount() * portTICK_PERIOD_MS / 1000),
                 resetReasonStr(r));
    if (haveDump) {
      DEBUG_PRINTI("HB coredump: task=%s PC=0x%08x exc_cause=%u tcb=0x%08x",
                   dump.exc_task, (unsigned)dump.exc_pc,
                   (unsigned)dump.ex_info.exc_cause, (unsigned)dump.exc_tcb);
      /* backtrace addresses (feed to xtensa-esp32s3-elf-addr2line) */
      for (int i = 0; i + 3 < (int)dump.exc_bt_info.depth && i < 12; i += 4) {
        DEBUG_PRINTI("HB bt[%d..%d]: 0x%08x 0x%08x 0x%08x 0x%08x", i, i + 3,
                     (unsigned)dump.exc_bt_info.bt[i],
                     (unsigned)dump.exc_bt_info.bt[i + 1],
                     (unsigned)dump.exc_bt_info.bt[i + 2],
                     (unsigned)dump.exc_bt_info.bt[i + 3]);
      }
    }
    vTaskDelay(M2T(5000));
  }
...
#ifdef BLIMP_MODE
  /* Also feed the mocap POSITION into the stock Kalman estimator as an external
   * measurement (Vicon-style). The filter fuses it with the IMU and produces a
   * clean, low-lag world VELOCITY, which the guidance reads back via
   * state->velocity -- replacing the old hand-rolled finite-difference. Position
   * only (no attitude) keeps it robust: no board-mount frame coupling. Small
   * stdDev = trust the mocap strongly. */
  positionMeasurement_t pos = { .x = pose8[0], .y = pose8[1], .z = pose8[2],
                                .stdDev = 0.01f };
...
void systemTask(void *arg)
{
  ...
  ledInit();
  ledSet(CHG_LED, 1);
  /* Start the post-mortem heartbeat FIRST so the reset reason is visible even
   * if a later init step hangs. Prints every 5 s on the USB console. */
  xTaskCreate(resetHeartbeatTask, "reset_hb", 4096, NULL, 1, NULL);
...
#ifdef BLIMP_MODE
  /* Use the KALMAN estimator so mocap position (fed in espnowMocapHandler) fuses
   * with the IMU into a usable world velocity for the guidance. The complementary
   * estimator has no external-position input, so it can't give us velocity. */
