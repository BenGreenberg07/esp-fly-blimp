...
/* ====================  ESP-FLY BLIMP MODE  ====================
 * A helium blimp is pendulum-stable, so it needs NO IMU stabilization.
 * This mode skips sensor calibration and the per-loop sensor wait, and maps
 * the operator's sticks straight to the motors at reduced power (the motors
 * just steer; the balloon provides lift). The disconnect failsafe is kept.
 * The motorPowerSet override (motor_test.py) still works in this mode too.
 * Comment out BLIMP_MODE to return to normal IMU-stabilized flight.
 */
#define BLIMP_MODE 1
/* Gains live in power_distribution_stock.c as CRTP params (group "blimp"),
 * so they can be tuned live over Wi-Fi without reflashing. */
extern float blimpFwdScale, blimpVertGain, blimpTurnGain;
/* On-board autonomous guidance (decoupled PID from mocap). When the host sets
 * CRTP param blimpc.autoEn=1 AND mocap data is fresh, the drone computes its
 * own forward/turn/vertical commands; otherwise it falls back to the manual
 * stick passthrough below. Default off => proven manual flight is unchanged. */
#include "blimp_guidance.h"
/* Swing-blimp build flag (4 canted motors, host-side Mellinger). Defaults to 0,
 * in which case nothing below changes. See blimp_swing.h. */
#include "blimp_swing.h"
/* ============================================================== */
...
static void stabilizerTask(void* param)
{
...
#else
  DEBUG_PRINTI("BLIMP_MODE: no-IMU blimp control active.\n");
#endif
  ...
  while(1) {
#ifdef BLIMP_MODE
    // Blimp: bypass the attitude PID (buoyancy self-rights it), but STILL read the
    // IMU and run the estimator so gyro/attitude telemetry and the client-side
    // yaw-hold have live data. (The calibration GATE is skipped above, which is
    // what previously blocked the motors — acquiring sensors here does not block.)
    //   thrust stick -> forward speed   (both forward motors)
    //   pitch  stick -> vertical (+climb = UP motor, -descend = DOWN motor)
    //   yaw    stick -> turn (differential on the forward motors)
    sensorsWaitDataReady();                          // ~1kHz pacing
    sensorsAcquire(&sensorData, tick);               // gyro.z etc. for telemetry
    stateEstimator(&state, &sensorData, &control, tick);  // stabilizer.roll/pitch/yaw
    compressState();
    tick++;
    ...
    if (emergencyStop || (systemIsArmed() == false)) {
      ...
      if (blimpGuidanceAutoEnabled() && blimpGuidanceMocapFresh(nowMs)) {
        /* AUTONOMOUS: drone computes forward/turn/vertical itself from the
         * mocap pose+target the host streamed into the `mocap` params. */
        static uint32_t lastAutoMs = 0;
        float dt = lastAutoMs ? (nowMs - lastAutoMs) * 0.001f : 0.02f;
        lastAutoMs = nowMs;
        blimpGuidanceUpdate(&control, &state, sensorData.gyro.z, dt);
        powerDistribution(&control);
      } else if (blimpGuidanceAutoEnabled()) {
        /* Auto selected but mocap is STALE -> failsafe: stop driving immediately
         * (the pendulum keeps it level; it just drifts/settles, no runaway).
         * Also clear the guidance latch + reset its integrators/trim/velocity
         * estimates right here (not just on the slower ESP-NOW packet-level
         * failsafe below) -- this is the FAST path: it fires the instant mocap
         * itself goes stale (bc_staleMs, tunable, default 300ms), rather than
         * waiting on the ESP-NOW link-level timeout. blimpGuidanceSetMocap()
         * re-engages automatically the instant a fresh pose arrives, so this
         * cannot prevent recovery -- it only guarantees a clean restart instead
         * of resuming with stale integrators after a signal gap. */
        powerStop();
        blimpGuidanceClearAuto();
      } else {
#if BLIMP_SWING
        /* SWING BLIMP: the four 0xA5 floats are already the four motor duties the
         * Mac's Mellinger controller + mixer computed. Pass them straight through
         * with NO gain applied -- blimpFwdScale/VertGain/TurnGain describe the
         * decoupled airframe's roles, which do not exist on this one. The swing
         * branch of powerDistribution() writes them to M1..M4 in order.
         * The on-board guidance branches above never run in this build: the host
         * streams only 0xA5 frames, never 0xA6 poses, so the auto latch is never set. */
        control.roll   = (int16_t)setpoint.attitude.roll;      /* M1 */
        control.pitch  = (int16_t)setpoint.attitude.pitch;     /* M2 */
        control.yaw    = (int16_t)setpoint.attitudeRate.yaw;   /* M3 */
        control.thrust = setpoint.thrust;                      /* M4 */
#else
        /* MANUAL passthrough (proven flight; used for ESP-NOW/Wi-Fi testing). */
        control.thrust = setpoint.thrust * blimpFwdScale;
        control.pitch  = (int16_t)(setpoint.attitude.pitch  * blimpVertGain);
        control.yaw    = (int16_t)(setpoint.attitudeRate.yaw * blimpTurnGain);
        control.roll   = 0;
#endif
        powerDistribution(&control);
      }
    ...
    if (testState != testDone) {
      ...
      /* ===============  ESP-FLY BLIMP OVERRIDE  ===============
       * A blimp is pendulum-stable from buoyancy, so we do NOT use the
       * IMU attitude PID. Map the raw operator setpoint straight onto
       * control_t, which power_distribution_stock.c turns into the
       * forward / turn / up / down motors.
       *
       *   thrust stick -> forward speed
       *   pitch  stick -> vertical (+climb / -descend)   [flip sign if reversed]
       *   yaw    stick -> turn (differential forward motors)
       *
       * Gains convert stick units (deg, deg/s) into motor PWM counts.
       */
      control.thrust = setpoint.thrust;                               // 0..60000
      control.pitch  = (int16_t)(setpoint.attitude.pitch  * 1000.0f); // ~deg -> PWM
      control.yaw    = (int16_t)(setpoint.attitudeRate.yaw *  100.0f); // ~deg/s -> PWM
      control.roll   = 0;
      /* ======================================================== */
