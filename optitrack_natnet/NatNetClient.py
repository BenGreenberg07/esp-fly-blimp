...
class NatNetClient:
    ...
    def __create_data_socket( self, port ):
        ...
        if self.use_multicast:
            ...
            result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            try:
                # Bind to '' (INADDR_ANY) before joining the group, and in that
                # order: Windows throws WinError 10049 if you bind directly to
                # the multicast address, and requires bind() before
                # IP_ADD_MEMBERSHIP. INADDR_ANY + bind-then-join also works on
                # macOS/Linux, so this is the cross-platform sequence.
                result.bind(('', port))
                mreq = socket.inet_aton(self.multicast_address) + socket.inet_aton(self.local_ip_address)
