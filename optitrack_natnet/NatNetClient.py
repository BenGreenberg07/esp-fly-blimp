...
class NatNetClient:
    ...
    def __create_data_socket( self, port ):
        ...
        if self.use_multicast:
            ...
            try:
                # macOS fix: a multicast receive socket must bind to the GROUP
                # address (or INADDR_ANY), NOT the host's unicast IP — binding to
                # local_ip_address silently receives nothing on macOS.
